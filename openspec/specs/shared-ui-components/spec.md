...
Which visual primitives `fulltime-ui` renders via `gpui-component` widgets vs. bespoke `gpui`
code, and the rule that a new shared primitive must use an existing `gpui-component` widget
when one covers the need before a custom one is written. Created by archiving change
use-gpui-component.

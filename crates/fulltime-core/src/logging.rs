...
//! - macOS:   `~/Library/Logs/com.pilgrimagesoftware.fulltime/`
//! - Linux:   `$XDG_DATA_HOME/com.pilgrimagesoftware.fulltime/logs/`
//! - Windows: `%APPDATA%\com.pilgrimagesoftware.fulltime\logs\`
//!
...
//!
//! When compiled with the `sentry` feature and `FULLTIME_SENTRY_DSN` is set
//! at runtime, ERROR-level `tracing` events are additionally forwarded to
...
pub fn init() -> LogGuards {
    ...
    let guards = if let Err(e) = std::fs::create_dir_all(&log_dir) {
        ...
        let file_appender = tracing_appender::rolling::daily(&log_dir, "fulltime.log");
...
fn init_sentry_client() -> (Option<sentry::ClientInitGuard>, SentryStatus) {
    let dsn = runtime_or_builtin(crate::constants::SENTRY_DSN_ENV,
                                 option_env!("FULLTIME_SENTRY_DSN_BUILTIN"));
    let environment = runtime_or_builtin(
        crate::constants::SENTRY_ENVIRONMENT_ENV,
        option_env!("FULLTIME_SENTRY_ENVIRONMENT_BUILTIN"),
    )
    .filter(|s| !s.is_empty())
    ...
    let release = runtime_or_builtin(
        ...
        option_env!("FULLTIME_SENTRY_RELEASE_BUILTIN"),
    )
    .filter(|s| !s.is_empty())
...
fn platform_log_dir() -> PathBuf {
    #[cfg(target_os = "macos")]
    {
        dirs::home_dir().map(|h| h.join("Library/Logs/com.pilgrimagesoftware.fulltime"))
                        .unwrap_or_else(|| PathBuf::from("/tmp/com.pilgrimagesoftware.fulltime"))
    }
    #[cfg(not(target_os = "macos"))]
    {
        dirs::data_local_dir()
            .map(|d| d.join("com.pilgrimagesoftware.fulltime/logs"))
            .unwrap_or_else(|| PathBuf::from("/tmp/com.pilgrimagesoftware.fulltime/logs"))
    }
}

...
//! brand mark, the league tab bar, the screen navigation control, and the
//! light/dark theme toggle — matching the Claude Design mockup's header,
//! which has no left sidebar.

use gpui::prelude::*;
use gpui::{Context, MouseButton, div, px};
use rust_i18n::t;

use crate::data::theme::{ColorTokens, FullTimeTheme, League, ThemeKey};
...
/// Renders the header row: brand, league tabs, screen nav, theme toggle.
pub fn render_header(colors: &ColorTokens, active_screen: AppScreen, active_league: League,
                     theme_key: ThemeKey, cx: &mut Context<RootView>)
                     -> impl IntoElement {
    ...
    div().h(px(56.0))
         ...
         .child(render_league_tabs(active_league, cx))
...
fn render_league_tabs(active_league: League, cx: &mut Context<RootView>) -> impl IntoElement {
    let selected_index = League::ALL.iter()
                                    .position(|l| *l == active_league)
                                    .unwrap_or(0);
    let labels: Vec<String> = League::ALL.iter().map(|l| l.label()).collect();

    render_tab_bar("header-league-tabs",
                   labels,
                   selected_index,
                   cx.listener(|this, ix: &usize, _window, cx| {
                         this.set_league(League::ALL[*ix], cx);
                     }))
}

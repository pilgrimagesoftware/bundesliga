...
//! target triple) as compile-time `rustc-env` variables, so an About panel
//! can show exactly which build is running.
//!
//! `fulltime-core` (the binary) depends on `fulltime-ui` (the library) — not
//! the reverse — so this crate needs its own build script rather than
//! reusing `fulltime-core/build.rs`'s Sentry-config forwarding.
...
fn main() {
    ...
    println!("cargo:rustc-env=FULLTIME_GIT_HASH={git_hash}");

    println!("cargo:rustc-env=FULLTIME_BUILD_DATE={}", build_date_utc());

    let target = std::env::var("TARGET").unwrap_or_else(|_| "unknown".to_string());
    println!("cargo:rustc-env=FULLTIME_TARGET={target}");
}
